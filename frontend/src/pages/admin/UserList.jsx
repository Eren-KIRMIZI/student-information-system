    ...
    <div className="animate-fade-in">
                ...
                <tbody>
                  {data.data.map((u) => (
                    <tr key={u.id}>
                      ...
                      <td>
                        {u.student
                          ? `${u.student.firstName} ${u.student.lastName} (Öğrenci)`
                          : u.lecturer
                            ? `${u.lecturer.firstName} ${u.lecturer.lastName} (Akademisyen)`
                            : 'Sistem Yöneticisi'}
                      </td>
                      <td>
                        <span className={`badge ${roleColor[u.role.name] || 'badge-gray'}`}>{u.role.name}</span>
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <StatusBadge status={u.isActive ? 'ACTIVE' : 'PASSIVE'} />
                      </td>
                      <td style={{ textAlign: 'right' }}>
                        <button className="btn btn-ghost btn-sm" onClick={() => setStatusConfirm(u)}>
                          <ShieldAlert size={14} />
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Yeni Kullanıcı Hesabı (Admin/Manuel)">
        <form onSubmit={handleSubmit(handleCreate)} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          >
            Not: Normalde öğrenciler Öğrenci Ekle sayfasından, Akademisyenler Akademisyen ekle sayfasından
            oluşturulduğunda sistem otomatik kullanıcı hesabı açar. Buradan doğrudan sistem yöneticisi (ADMIN) veya
            bağımsız hesaplar açabilirsiniz.
          </div>
