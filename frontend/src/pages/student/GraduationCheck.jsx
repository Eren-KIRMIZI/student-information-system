      ...
      <div className="card" style={{ padding: 24, marginBottom: 20, textAlign: 'center' }}>
        {canGraduate ? (
          <>
            ...
            <h2 style={{ margin: 0, color: '#059669', fontWeight: 800, fontSize: 22 }}>
              TEBRİKLER! Mezuniyet Koşullarını Sağlıyorsunuz
            </h2>
          </>
        ) : (
          <>
            <AlertTriangle size={48} color="#d97706" style={{ margin: '0 auto 12px' }} />
            <h2 style={{ margin: 0, color: '#d97706', fontWeight: 800, fontSize: 22 }}>
              Henüz Mezuniyet Koşullarını Sağlamıyorsunuz
            </h2>
          </>
        )}
      </div>
        ...
        {Object.entries(checks).map(([key, check]) => {
          ...
          return (
            <div
              key={key}
              className="card"
              style={{ padding: 20, borderLeft: `4px solid ${check.met ? '#059669' : '#dc2626'}` }}
            >
              ...
              <div style={{ fontSize: 24, fontWeight: 800, color: check.met ? '#059669' : '#dc2626' }}>
                ...
                {key === 'gpa' ? null : (
                  <span style={{ fontSize: 16, fontWeight: 400, color: 'var(--color-text-muted)' }}>
                    {' '}
                    {check.required || check.minimum}
                  </span>
                )}
              </div>
              {key === 'gpa' && (
                <div style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>Min: {check.minimum}</div>
              )}
            </div>
          );
        })}
