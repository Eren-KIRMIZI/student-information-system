    ...
    <div className="animate-fade-in">
      ...
      >
        ...
        <div className="card" style={{ textAlign: 'center', padding: 20 }}>
          <AlertTriangle
            size={28}
            color={data.conflicts?.length ? '#dc2626' : '#059669'}
            style={{ margin: '0 auto 8px' }}
          />
          <div style={{ fontSize: 28, fontWeight: 800, color: data.conflicts?.length ? '#dc2626' : '#059669' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {data.conflicts.map((c, i) => (
              ...
              >
                ...
                <span>
                  {dayLabels[c.slot1.dayOfWeek]} {c.slot1.startTime}-{c.slot1.endTime}
                </span>
                <span style={{ color: 'var(--color-text-muted)' }}>&</span>
                <span style={{ fontWeight: 600, color: '#dc2626' }}>{c.slot2.courseCode}</span>
                <span>
                  {dayLabels[c.slot2.dayOfWeek]} {c.slot2.startTime}-{c.slot2.endTime}
                </span>
              </div>
            ))}
          </div>
