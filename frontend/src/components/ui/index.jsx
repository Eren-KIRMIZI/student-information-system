        ...
        {Array.from({ length: rows }).map((_, i) => (
          <tr key={i}>
            {Array.from({ length: cols }).map((_, j) => (
              <td key={j}><Skeleton height={16} width={j === 0 ? '60%' : '80%'} /></td>
            ))}
          </tr>
        ))}
      </tbody>
...
export const ErrorState = ({ message = 'Veri yüklenirken bir hata oluştu', onRetry }) => (
  <div className="error-state">
    ...
    <h3>Hata</h3>
    <p>{message}</p>
    {onRetry && <button onClick={onRetry} className="btn btn-secondary" style={{ marginTop: 8 }}>Tekrar Dene</button>}
  </div>
);
...
export const Pagination = ({ page, limit, total, totalPages, onPageChange }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderTop: '1px solid var(--color-border)', fontSize: 13, color: 'var(--color-text-secondary)' }}>
      <span>Toplam <strong>{total}</strong> kayıt, {totalPages} sayfa</span>
      <div style={{ display: 'flex', gap: 4 }}>
        <button onClick={() => onPageChange(page - 1)} disabled={page <= 1} className="btn btn-ghost btn-sm">‹</button>
        ...
        <button onClick={() => onPageChange(page + 1)} disabled={page >= totalPages} className="btn btn-ghost btn-sm">›</button>
      </div>
    </div>
  );
};
...
export const SearchInput = ({ value, onChange, placeholder = 'Ara...' }) => {
  ...
  return (
    <div style={{ position: 'relative', minWidth: 220 }}>
      ...
      />
      {local && (
        <button onClick={() => { setLocal(''); onChange(''); }} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', display: 'flex' }}>
          <X size={14} />
        </button>
      )}
    </div>
...
// ====== ConfirmDialog ======
export const ConfirmDialog = ({ open, isOpen, title, message, description, onConfirm, onCancel, onClose, danger = true }) => {
    ...
    <div className="modal-overlay" onClick={handleCancel}>
      <div className="modal-box confirm-dialog" onClick={(e) => e.stopPropagation()}>
        <div style={{ textAlign: 'center' }}>
          ...
          <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 8, color: 'var(--color-text-primary)' }}>{title}</h3>
          <p style={{ fontSize: 14, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>{text}</p>
        </div>
        <div className="confirm-actions">
          <button onClick={handleCancel} className="btn btn-secondary">İptal</button>
          <button onClick={onConfirm} className={`btn ${danger ? 'btn-danger' : 'btn-primary'}`}>Onayla</button>
        </div>
      </div>
    </div>
  );
  ...
  <div className="tabs-list">
    {tabs.map((tab) => (
      ...
      >
        {tab.label}
        {tab.badge !== undefined && (
          <span className="badge badge-blue" style={{ marginLeft: 6, padding: '1px 6px', fontSize: 11 }}>{tab.badge}</span>
        )}
      </button>
    ))}
  </div>
