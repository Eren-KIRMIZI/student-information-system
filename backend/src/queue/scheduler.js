...
export const setupScheduledJobs = async () => {
  await scheduledQueue.add('cleanup-expired-refresh-tokens', {}, {
    repeat: { cron: '0 3 * * *' },
    jobId: 'cleanup-refresh-tokens',
  });

  await scheduledQueue.add('cleanup-expired-idempotency', {}, {
    repeat: { cron: '0 4 * * *' },
    jobId: 'cleanup-idempotency',
  });

  await scheduledQueue.add('generate-semester-reports', {}, {
