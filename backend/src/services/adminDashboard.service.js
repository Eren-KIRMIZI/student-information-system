...
export const getAdminKPIs = async () => {
  const [totalStudents, totalLecturers, totalCourses, enrollmentStats, gradeDistribution, departmentStats] =
    await Promise.all([
  ...
  const gpaDistribution = await prisma.student.groupBy({
    by: ['departmentId'],
    _avg: { gpa: true },
    _count: { id: true },
  });
  ...
  return {
    ...
    gpaByDepartment: gpaDistribution
      .filter((g) => g.departmentId)
      .map((g) => ({
        departmentId: g.departmentId,
        avgGpa: g._avg.gpa ? Math.round(g._avg.gpa * 100) / 100 : 0,
