...
export async function exportStudents(res) {
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="ogrenciler-${new Date().toISOString().slice(0, 10)}.xlsx"`,
  );
...
export async function exportAttendance(res, filters = {}) {
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="devamsizlik-${new Date().toISOString().slice(0, 10)}.xlsx"`,
  );
...
export async function exportTranscript(res, studentId) {
  ...
  sheet.getCell('A3').value = 'Öğrenci Adı:';
  sheet.getCell('B3').value = `${student.firstName} ${student.lastName}`;
  sheet.getCell('A4').value = 'Öğrenci No:';
  sheet.getCell('B4').value = student.studentNumber;
  sheet.getCell('A5').value = 'Bölüm:';
  sheet.getCell('B5').value = student.department?.name || '-';
  sheet.getCell('D3').value = 'GPA:';
  sheet.getCell('E3').value = student.gpa ?? '-';
  sheet.getCell('D4').value = 'Toplam Kredi:';
  sheet.getCell('E4').value = student.totalCredits ?? '-';
  sheet.getCell('D5').value = 'Tarih:';
  sheet.getCell('E5').value = new Date().toLocaleDateString('tr-TR');
