...
export const getAdvisorStudents = async (userId) => {
  const lecturer = await import('../config/prisma.js').then(m => m.default.lecturer.findUnique({ where: { userId }, select: { id: true } }));
  ...
  const studentsWithRisk = advisees.map(a => {
    ...
    const avgAttendance = activeEnrollments.length > 0 ? activeEnrollments.reduce((s, e) => s + (e.attendance?.length || 0), 0) / activeEnrollments.length : 0;

    const riskLevel = riskFlags.length >= 2 ? 'HIGH' : riskFlags.length === 1 ? 'MEDIUM' : 'LOW';

    return { id: s.id, firstName: s.firstName, lastName: s.lastName, studentNumber: s.studentNumber, department: s.department.name, classYear: s.classYear, gpa, totalCredits: totalCred, activeCourses: activeEnrollments.length, failedCourses, riskLevel, riskFlags };
  });

  return { advisees: studentsWithRisk, summary: { total: studentsWithRisk.length, highRisk: studentsWithRisk.filter(s => s.riskLevel === 'HIGH').length, mediumRisk: studentsWithRisk.filter(s => s.riskLevel === 'MEDIUM').length, lowRisk: studentsWithRisk.filter(s => s.riskLevel === 'LOW').length } };
};
...
export const getAdvisorStudentDetail = async (userId, studentId) => {
  ...
  const assignment = await import('../config/prisma.js').then(m => m.default.advisorAssignment.findFirst({ where: { lecturerId: lecturer.id, studentId, isActive: true } }));
