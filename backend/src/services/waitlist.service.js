...
export const promoteFromWaitlist = async (waitlistEntryId) => {
  const entry = await import('../config/prisma.js').then((m) =>
    m.default.waitlist.findUnique({
  ...
  const duplicate = await import('../config/prisma.js').then((m) =>
    m.default.enrollment.findFirst({
      where: { studentId: entry.studentId, courseSectionId: section.id, status: { not: 'REJECTED' } },
    }),
  );
  if (duplicate) throw new AppError('Öğrenci zaten bu derse kayıtlı', 409);

  await import('../config/prisma.js').then((m) =>
    m.default.enrollment.create({
      data: { studentId: entry.studentId, courseSectionId: section.id, status: 'ACTIVE' },
    }),
  );
  ...
  try {
    getIO()
      .to(`student:${entry.student.userId}`)
      .emit('enrollment:promoted', { courseSectionId: section.id, courseName: section.course.name });
  } catch {}
