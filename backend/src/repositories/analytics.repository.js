...
export const studentFindById = (id) => prisma.student.findUnique({ where: { id }, include: { department: true } });
  ...
  prisma.advisorAssignment.findMany({
    ...
    include: {
      student: {
        include: {
          department: true,
          enrollments: { include: { courseSection: { include: { course: true } }, grade: true } },
        },
      },
    },
  });

export const getAllStudents = () => prisma.student.findMany({ include: { department: true } });

export const getDepartmentStats = () => prisma.student.groupBy({ by: ['departmentId'], _count: { id: true } });

export const getEnrollmentStats = () => prisma.enrollment.groupBy({ by: ['status'], _count: { id: true } });
...
export const getSectionsWithEnrollments = () =>
  prisma.courseSection.findMany({
    include: {
      course: true,
      enrollments: { where: { status: { in: ['ACTIVE', 'APPROVED'] } } },
