...
export const studentFindById = (id) =>
  prisma.student.findUnique({
    ...
    include: {
      ...
      advisorHistory: { include: { lecturer: true }, where: { isActive: true } },
      enrollments: {
...
export const studentCreate = (data) =>
  prisma.student.create({ data, include: { department: true, user: { select: { email: true } } } });
export const studentUpdate = (id, data) =>
  prisma.student.update({ where: { id }, data, include: { department: true } });
export const studentUpdateStatus = (id, isActive) =>
  prisma.user.update({
    where: {
      id: (async () => {
        const s = await prisma.student.findUnique({ where: { id }, select: { userId: true } });
        return s.userId;
      })(),
    },
    data: { isActive },
...
export const lecturerFindById = (id) =>
  prisma.lecturer.findUnique({
    where: { id },
    include: {
      department: { include: { faculty: true } },
      user: { select: { email: true, isActive: true } },
      sections: { include: { course: true }, take: 5, orderBy: { createdAt: 'desc' } },
    },
  });
export const lecturerFindByUserId = (userId) => prisma.lecturer.findUnique({ where: { userId } });
export const lecturerCreate = (data) =>
  prisma.lecturer.create({ data, include: { department: true, user: { select: { email: true } } } });
export const lecturerUpdate = (id, data) =>
  prisma.lecturer.update({ where: { id }, data, include: { department: true } });
