...
export const deptFindMany = (params) => {
  ...
  return Promise.all([
    prisma.department.findMany({
      ...
      take,
      orderBy: { [sortBy]: order },
      include: { faculty: true, _count: { select: { students: true, lecturers: true, courses: true } } },
    }),
...
export const deptHasDependents = (id) =>
  Promise.all([
...
export const courseFindMany = (params) => {
  ...
  return Promise.all([
    prisma.course.findMany({
      ...
      take,
      orderBy: { [sortBy]: order },
      include: { department: { include: { faculty: true } }, _count: { select: { sections: true } } },
    }),
...
export const courseFindById = (id) =>
  prisma.course.findUnique({
    where: { id },
    include: { department: { include: { faculty: true } }, sections: { include: { lecturer: true } } },
  });
...
export const sectionFindMany = async (params) => {
  ...
  const [rows, total] = await Promise.all([
    prisma.courseSection.findMany({
      ...
      take,
      orderBy: { [sortBy]: order },
      include: { course: { include: { department: true } }, lecturer: true, _count: { select: { enrollments: true } } },
    }),
...
export const sectionFindById = async (id) => {
  const s = await prisma.courseSection.findUnique({
    where: { id },
    include: {
...
export const sectionUpdate = (id, data) =>
  prisma.courseSection.update({ where: { id }, data, include: { course: true, lecturer: true } });
...
export const sectionHasEnrollments = (id) =>
  prisma.enrollment.count({ where: { courseSectionId: id, status: { in: ['ACTIVE', 'PENDING'] } } });
