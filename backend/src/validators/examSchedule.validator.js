...
export const createExamSlotValidator = [
  body('examType')
    .trim()
    .notEmpty()
    .withMessage('Sinav türü zorunludur')
    .isIn(EXAM_TYPES)
    .withMessage('Geçersiz sinav türü'),
  body('examDate').trim().notEmpty().withMessage('Sinav tarihi zorunludur').isISO8601().withMessage('Geçersiz tarih'),
  body('startTime')
    .trim()
    .notEmpty()
    .withMessage('Baslangic saati zorunludur')
    .matches(/^\d{2}:\d{2}$/)
    .withMessage('Saat formati: SS:DD'),
  body('endTime')
    .trim()
    .notEmpty()
    .withMessage('Bitis saati zorunludur')
    .matches(/^\d{2}:\d{2}$/)
    .withMessage('Saat formati: SS:DD'),
